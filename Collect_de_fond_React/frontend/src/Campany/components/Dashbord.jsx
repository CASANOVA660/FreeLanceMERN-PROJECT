  ...
  useEffect(() => {
    const fetchUserStats = async () => {
      try {
        ...
        const campaignsRes = await axios.get('http://localhost:5000/api/campaigns/count');
        const donationsRes = await axios.get('http://localhost:5000/api/donations/count');
        const usersRes = await axios.get('http://localhost:5000/api/auth/count');

        
        // Construction de l'objet avec la structure exacte des réponses
        const statsData = {
          totalCampaigns: Number(campaignsRes.data.totalCampaigns), // la réponse est directement le nombre
          totalDonations: Number(donationsRes.data.totalDonationsCount), // la réponse est dans .total
          activeUsers: Number(usersRes.data.totalUser) // la réponse est directement le nombre
        };
