...
const API_URLL = 'http://localhost:8082/api/campaigns'; 
/*const API_URLLD = 'http://localhost:8082/api/donations'; */
...
export const createAssociation = async (associationData) => {
  try {
    const response = await axios.post('http://localhost:8082/api/associations/create', associationData);
    return response.data; // Renvoie les données renvoyées par l'API
  } catch (error) {
...
export const makeDonation = async (donationData) => {
  try {
    ...
    const response = await axios.post('http://localhost:8082/api/donations/donate', payload);
    return response.data;
  } catch (error) {
...
export const deleteCampaign = async (id) => {
  try {
    const response = await axios.delete(`http://localhost:8082/api/campaigns/${id}`);
    return response.data; 
  } catch (error) {
...
export const fetchDonations = async (email) => {
  try {
    ...
    const response = await axios.get(`http://localhost:8082/api/donations/by-email`, {
      params: {
        email: email
      }
    });
