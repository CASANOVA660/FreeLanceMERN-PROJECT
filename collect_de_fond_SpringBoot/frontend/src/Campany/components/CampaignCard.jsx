...
const CampaignCard = ({ campaign, onEdit, onUpdate }) => {
  ...
  const handleDelete = async () => {
    try {
      if (!campaign.id) {  // Changé de _id à id
        toast.error("ID de campagne non valide");
        return;
      }
  
      await deleteCampaign(campaign.id);  // Changé de _id à id
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const formattedData = {
        ...
        association: {
          id: campaign.association.id  // Ajout de l'ID de l'association
        }
      };
  
      console.log('Données formatées:', formattedData); // Pour le débogage
  
      await updateCampaign(campaign.id, formattedData);  // Changé de _id à id
